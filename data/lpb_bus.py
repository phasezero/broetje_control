...
def read(r):
  ...
  for i in range(int(r)):
    a=ser.readall()
    if a != b'':
      l.append([a^255,time.time()])
  return(l)

def zeit(raw):
  n=1
  e=[]
  for z in range(len(raw)-1):
    e.append(['Zeitunterschied {}. zu {}.: {}'.format(n,n+1,raw[n][1]-raw[z][1]),raw[n][1]-raw[z][1]])
    n += 1
  n=1
  max=0
  min=1
  s=[]
  c=0
  for y in range(len(e)-1):
    print(e[y][0])
    if e[y][1] > max: max = e[y][1]
    if e[y][1] < min: min = e[y][1]
    if e[y][1] > 0.007:
      s.append(e[y][1])
      c += 1
  for u in s:
    print(u)
  print('Gesamt: {}'.format(c))
...
def test():
  ...
  while True:
    ...
    if a:
      ...
      t3=t1-t2
      if t3 > 0.0075:
        print('{}: Byteanzahl: {}, {}, Time: {}'.format(q,len(c),repr(c),t3))
        c=[]
    t2=t1
    if a:
      c.append(a[0]^255)
